...
def cnp_jotas(conc):
    ...
    conc.drop([41,105,106], inplace=True) 
...
def c_onc(form_s):
    ...
    conc_1 = pd.concat([ fix_prospect(s_tack(*set_state(i))) for i in range(3)])
    def gambiarra(new_mg):
        ...
        return new_mg
    conc_2 =  gambiarra(s_tack(*set_state(3)))
    conc   = pd.concat([conc_1,conc_2])    
    ...
    conc.loc[:,col] = conc.loc[:,col].str.title()
...
def update(conc):
    ...
    save_pkl(conc,'conc_2s')
    conc.to_sql('forms_concs',con,if_exists='replace')
    conc.to_excel('C:/Users/Mucho/Work/mac/Target/Pickles/reports/conc_ss.xlsx')
    con.commit()
...
def magic():
    ...
    m = zb_link(dat_es(ac_tions(cnp_jotas(c_onc(forms)))))
